...
double Ecin;
    ...
    void set_temperature(double t){if(t>0) temperature = t;}
       ...
       double get_Ecin() const {return Ecin;}

       double get_pression()const {return pression;}

       void change_couleur();

       void change_trace(){if (particules[1]->get_trace()){particules[1]->set_trace(false);}
                                     else{particules[1]->set_trace(true);}}

       void change_pas(bool b){if(b){pas=pas+0.001;}
           else{pas=pas-0.001;}}
