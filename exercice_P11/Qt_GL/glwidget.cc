...
//c.ajouter_particule(new Neon(1, 18.5, 1 ,0 ,10, 0 ,20.1797));
//c .ajouter_particule(new Argon(1, 1, 3.1 ,0, 0, -0.5,39.948));

c.initialise_rd_neon(500,10);
...
double GLWidget::info(int i) const{if (i==0){return c.get_Ecin();}
    if (i==1){return c.get_comptechocparois();}
   if (i==2){return c.get_pression();}
   if (i==3){return c.get_pas();}
   return 0;}
