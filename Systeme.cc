...
void Systeme::evolue2(double dt){
    ...
    for(size_t i(0); i < particules.size(); ++i){//on regarde si la case dans laquelle se trouve chaque particule en contient au moins une autre
        size_t X = particules[i]->num_kase(0);
        size_t Y = particules[i]->num_kase(1);
        size_t Z = particules[i]->num_kase(2);
        std::vector<size_t> kase = E.get_kase()[X][Y][Z];
            if( kase.size() > 1){
                unsigned int p1;
                unsigned int p2;
                ...
                else{//si plus de 2 particules dans la meme case on tire au hasard les 2 qui se rencontrent
                    do{
